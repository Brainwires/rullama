...
fn main() -> ExitCode {
    ...
    let messages = vec![
        ChatMessage {
            ...
            content: "Transcribe the following audio exactly as spoken. \
                      Output only the transcription text, nothing else.".into(),
        },
        ChatMessage {
    ...
    let (audio_begin, _audio_end) = model.audio_sentinel_ids_native()
    ...
    println!("prompt-eval done in {:?}; first sampled token = {} ({:?})",
    ...
    println!("rullama generated {} tokens in {:?}: {out:?}",
        out_ids.len(), t.elapsed());
    println!("rullama ids: {out_ids:?}");

    // ---- Ollama side ----
    println!("\n== ollama side ==");
    let out = Command::new("curl")
        .args(["-s", "-X", "POST", "http://localhost:11434/v1/audio/transcriptions",
