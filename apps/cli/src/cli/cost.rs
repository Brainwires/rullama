...
pub async fn handle_cost(period: Option<String>, reset: bool) -> Result<()> {
    ...
    // Default to today so a quick `rullama cost` after a prompt shows data.
    ...
    if summary == "No usage data" {
        ...
        // `StreamChunk::Usage` event. Some transports (notably the rullama
        ...
        println!(
            "\nNo usage data for period '{}'.\n\n\
             ...
             - The rullama SaaS provider does not always forward per-request \
               token counts from upstream; try a direct provider \
               (`--provider anthropic` or `--provider openai`) to verify.\n  \
             - Try another period: `rullama cost --period week|month|30days`.\n",
            period
        );
    } else {
        println!("\n{}", summary);
