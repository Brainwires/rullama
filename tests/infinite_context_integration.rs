...
async fn test_message_storage_and_search() -> Result<()> {
    ...
    let client = Arc::new(LanceClient::new(db_path.to_str().unwrap()).await?);
    ...
    assert_eq!(first_msg.message_id, "msg1", "Should find the JWT question as most relevant");
    assert!(*first_score > 0.70, "JWT query should have high relevance score: {}", first_score);

    // Second result should be the JWT response
    if results.len() > 1 {
        let (second_msg, second_score) = &results[1];
        assert_eq!(second_msg.message_id, "msg2", "Should find JWT response as second result");
        assert!(*second_score > 0.70, "JWT response should be relevant: {}", second_score);
    }

    // Test 2: Irrelevant queries should NOT return JWT messages
    let irrelevant_results = store.search_conversation(
        "conv_test",
    ...
    if !irrelevant_results.is_empty() {
        ...
        assert_eq!(top_result.message_id, "msg3", "Weather query should find weather message");
    }
    ...
    assert!(!all_results.is_empty(), "Should find authentication messages across all conversations");
...
async fn test_embedding_cache() -> Result<()> {
    ...
    println!("   - Speedup: {:.1}x", cold_latency.as_micros() as f64 / hot_latency.as_micros().max(1) as f64);

    // Cached should be significantly faster (at least 10x)
    assert!(hot_latency < cold_latency / 10,
        "Cached embedding should be much faster: cold={:?}, hot={:?}",
        cold_latency, hot_latency);
...
async fn test_conversation_isolation() -> Result<()> {
    ...
    let conv1_results = store.search_conversation(
        "conversation_1",
        "database choice",
        10,
        0.3,
    ...
    assert!(!conv1_results.is_empty(), "Should find messages in conversation 1");
    assert_eq!(conv1_results[0].0.conversation_id, "conversation_1");
    assert!(conv1_results[0].0.content.contains("PostgreSQL"),
        "Should find PostgreSQL message, not MongoDB");

    // Search within conversation 2 only
    let conv2_results = store.search_conversation(
        "conversation_2",
        "database choice",
        10,
        0.3,
    ...
    assert!(!conv2_results.is_empty(), "Should find messages in conversation 2");
    assert_eq!(conv2_results[0].0.conversation_id, "conversation_2");
    assert!(conv2_results[0].0.content.contains("MongoDB"),
...
async fn test_relevance_thresholds() -> Result<()> {
    ...
    let high_threshold = store.search_conversation(
        "test",
    ...
    for (msg, score) in &high_threshold {
        println!("  - [{}] {}: {}", score, msg.message_id, &msg.content[..60.min(msg.content.len())]);
    }

    // Test with medium threshold (0.6) - should get exact + partial
    let med_threshold = store.search_conversation(
        "test",
        "rate limiting token bucket",
        10,
        0.6,
    ...
    let low_threshold = store.search_conversation(
        "test",
        "rate limiting token bucket",
        10,
        0.4,
    ...
    assert!(high_threshold.len() <= med_threshold.len(),
        "Higher threshold should return fewer results");
    assert!(med_threshold.len() <= low_threshold.len(),
...
async fn bench_search_latency() -> Result<()> {
    ...
    // Warm up
    let _ = store.search_conversation("bench_test", "database", 5, 0.6).await?;
    ...
    for _ in 0..iterations {
        let start = Instant::now();
        let _ = store.search_conversation("bench_test", "authentication system design", 5, 0.6).await?;
    ...
    assert!(avg_latency.as_millis() < 200,
        "Search should complete in < 200ms, took {:?}", avg_latency);
