...
chat (Ollama Cloud + OpenAI) in production. The Worker lives in this directory (`services/worker/`).
...
Edit [`wrangler.toml`](wrangler.toml) → `[vars] CORS_ORIGINS` to
...
Edit `src/index.ts` (in this dir), then `cd services/worker && npx wrangler deploy`. It's
