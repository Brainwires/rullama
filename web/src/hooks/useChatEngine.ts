...
import { streamCloudChat, type CloudChatMessage } from "@/lib/cloud/client";
import { getCloudKey } from "@/lib/cloud/keyvault";
import type { CloudModel } from "@/lib/cloud/types";
import {
    INFLIGHT_KEY,
...
export interface UseChatEngineParams {
     ...
     *  null for local/diffusion. Drives the cloud branch in runJob. */
    loadedCloud: CloudModel | null;
    /** Power-user proxy base-URL override (their own Cloudflare Worker). */
...
export function useChatEngine(opts: UseChatEngineParams) {
    ...
    // Read the proxy override at runJob time without churning runJob's identity.
    const cloudBaseOverrideRef = useRef(cloudBaseOverride);
    cloudBaseOverrideRef.current = cloudBaseOverride;
    ...
    const runJob = useCallback(async (job: GenJob) => {
        ...
        try {
            ...
            // ── Cloud chat (BYOK proxy) ────────────────────────────────────
            // No session/Model/KV — pure HTTP streaming through /api/cloud/*.
            // Build OpenAI-style messages from the SAME sysContent + history,
            // stream deltas into the bubble, and reuse the SAME text <tool_call>
            // loop + orchestrator as local: search_knowledge + the weather/etc
            // tools run locally via executeTool, so local-embeddings RAG works
            // with a cloud chat model. Cloud is text-only (onSend blocks media).
            if (cloudTurn) {
                const provider = cloudTurn.provider;
                const apiKey = await getCloudKey(provider);
                if (!apiKey) {
                    const note = `No ${provider} API key — add it in Settings → Cloud.`;
                    displayHistory[displayHistory.length - 1].content = note;
                    reflectDisplay(convId, modelMsgId, displayHistory);
                    if (convId && modelMsgId) { try { await client.msgSetContent(convId, modelMsgId, note); } catch { /* */ } }
                    return;
                }
                beacon("chat", `cloud start ${provider}:${cloudTurn.model}`);
                const ac = new AbortController();
                const cancelPoll = setInterval(() => { if (cancelRef.current) ac.abort(); }, 200);
                let lastFlushAt = performance.now();
                const flushBubble = async () => {
                    if (!convId || !modelMsgId) return;
                    try { await client.msgSetContent(convId, modelMsgId, displayHistory[displayHistory.length - 1].content); } catch { /* */ }
                };
                // Stream one cloud completion, appending deltas to the bubble.
                // Returns the text generated by THIS call (one assistant turn).
                const streamInto = async (msgs: CloudChatMessage[]): Promise<string> => {
                    let turnText = "";
                    let first = true;
                    // Cloud (esp. a 31B model cold-starting) can take many seconds
                    // to the FIRST token; show activity so it doesn't read as a hang.
                    setStatusLine(`contacting ${provider}…`);
                    for await (const delta of streamCloudChat({
                        provider, model: cloudTurn.model, apiKey, messages: msgs,
                        sampling, maxTokens, proxyBase: cloudBaseOverrideRef.current, signal: ac.signal,
                    })) {
                        if (cancelRef.current) break;
                        if (first) { first = false; setStatusLine(undefined); }
                        turnText += delta;
                        displayHistory[displayHistory.length - 1].content += delta;
                        reflectDisplay(convId, modelMsgId, displayHistory);
                        if (performance.now() - lastFlushAt > 500) { lastFlushAt = performance.now(); await flushBubble(); }
                    }
                    if (first) setStatusLine(undefined); // stream produced nothing
                    return turnText;
                };

                // Growing OpenAI conversation: system + prior turns + user. Each
                // tool round appends an assistant turn (model text) + a user turn
                // (the tool-response block) so the model sees results and continues.
                const convo: CloudChatMessage[] = [];
                if (sysContent) convo.push({ role: "system", content: sysContent });
                for (const m of userTurns) {
                    convo.push({ role: m.role === "model" ? "assistant" : "user", content: m.content });
                }
                convo.push({ role: "user", content: text });

                try {
                    let modelText = await streamInto(convo);

                    // Orchestrator (Programmatic Tool Calling) — backend-agnostic:
                    // parse a Rhai script from the text and run it locally. On
                    // failure, re-stream a JSON-loop correction via cloud.
                    ...
                    if (toolMode && orchestratorMode && !cancelRef.current) {
                        ...
                        if (script) {
                            ...
                            const res = await runOrchestration(script, { weatherApiKey, newsApiKey, units: weatherUnits, geo, conversationId: convId ?? null });
                            setStatusLine(undefined);
                            if (res.ok) {
                                ...
                                reflectDisplay(convId, modelMsgId, displayHistory);
                                const note =
                                    `That script could not run (${res.error}). Do NOT write a script. ` +
                                    `Answer now; if a tool is needed emit ` +
                                    `<tool_call>{"name": "<tool>", "arguments": { ... }}</tool_call> blocks.`;
                                convo.push({ role: "assistant", content: modelText });
                                convo.push({ role: "user", content: note });
                                modelText = await streamInto(convo);
                            }
                        }
                    }

                    // Text <tool_call> loop — identical contract to local; tools
                    // run via executeTool (search_knowledge → local embedder).
                    ...
                    for (let round = 0; toolMode && !orchestratedOk && !cancelRef.current && round < MAX_TOOL_ROUNDS; round++) {
                        ...
                        const results = await Promise.all(
                            execCalls.map((c) => executeTool(c.name, c.arguments, {
                                weatherApiKey, newsApiKey, units: weatherUnits, geo, conversationId: convId ?? null,
                            })),
                        );
                        let respBlock = "\n";
                        execCalls.forEach((c, i) => { respBlock += toolResponseBlock(c.name, results[i].summary) + "\n"; });
                        displayHistory[displayHistory.length - 1].content += respBlock;
                        reflectDisplay(convId, modelMsgId, displayHistory);
                        setStatusLine(undefined);
                        if (cancelRef.current) break;
                        // Continue: feed the model's prior text + the tool results
                        // as a user turn, stream the continuation into the bubble.
                        convo.push({ role: "assistant", content: modelText });
                        convo.push({ role: "user", content: respBlock });
                        modelText = await streamInto(convo);
                    }

                    await flushBubble();
                    // Surface a silent empty completion instead of leaving a blank
                    // bubble that looks like a hang.
                    if (!cancelRef.current && !displayHistory[displayHistory.length - 1].content.trim()) {
                        showToast({ level: "warn", title: "Empty response", message: `${provider} returned no text — check the model id and your API key/limits.` });
                    }
                    if (convId) {
                        try { await client.convTouch(convId, suggestTitle(text)); await client.dbFlush(); } catch { /* */ }
                        void refreshConversations();
                    }
                    beacon("chat", `cloud gen done (${provider}:${cloudTurn.model}, ${displayHistory[displayHistory.length - 1].content.length} chars)`);
                } catch (e) {
                    const msg = (e as Error)?.message ?? String(e);
                    if (!/abort/i.test(msg)) {
                        showToast({ level: "error", title: "Cloud request failed", message: msg });
                    }
                    await flushBubble();
                } finally {
    ...
    const onSend = useCallback(async () => {
        ...
        // Cloud chat is text-only in v1 — refuse a turn with attachments rather
        // than silently dropping them.
        if (loadedCloud && (turnImages.length > 0 || turnAudio.length > 0)) {
            showToast({ level: "warn", title: "Cloud models are text-only", message: "Remove the attachment, or switch to a local model for vision/audio." });
            return;
        }
        const client = getClient();
        ...
        const job: GenJob = {
            ...
            cloud: loadedCloud ? { provider: loadedCloud.provider, model: loadedCloud.model } : undefined,
            modelDigest: lastLoadedDigest,
            images: turnImages,
            audio: turnAudio,
            status: "queued",
        ...
        activeConvId, commitQueue, kickPump, lastLoadedDigest, loadedIsDiffusion, loadedCloud,
