...
use brainwires::permissions::{
    ActionOutcome, AuditEvent, AuditEventType, AuditLogger,
    PolicyAction, PolicyEngine, PolicyRequest, TrustLevel,
};
...
use super::{AgentPoolTool, BashTool, CodeExecTool, ContextRecallTool, FileOpsTool, GitTool, McpToolExecutor, OrchestratorTool, PlanTool, SearchTool, SemanticSearchTool, SessionTaskTool, TaskManagerTool, ToolRegistry, ToolSearchTool, WebTool};
...
pub struct ToolExecutor {
    ...
    remote_bridge: Option<std::sync::Arc<tokio::sync::RwLock<brainwires::agent_network::remote::RemoteBridge>>>,
...
impl ToolExecutor {
    ...
    pub fn set_remote_bridge(
        &mut self,
        bridge: std::sync::Arc<tokio::sync::RwLock<brainwires::agent_network::remote::RemoteBridge>>,
    ) {
    ...
    pub fn apply_org_policies(&mut self, policies: &brainwires::agent_network::remote::OrgPolicies) {
    ...
    pub fn set_approval_channel(&mut self, tx: tokio::sync::mpsc::Sender<crate::approval::ApprovalRequest>) {
    ...
    pub fn set_sudo_password_channel(&mut self, tx: tokio::sync::mpsc::Sender<crate::sudo::SudoPasswordRequest>) {
    ...
    async fn request_sudo_password(&self, command: &str) -> Option<zeroize::Zeroizing<String>> {
        ...
        if let Some(tx) = &self.sudo_password_tx {
            ...
            tokio::task::spawn_blocking(move || {
                ...
                let cmd_display = if cmd.len() > 40 { format!("{}...", &cmd[..40]) } else { cmd };
                let prompt = format!("[sudo] password for {} (command: {})", username, cmd_display);
    ...
    pub fn record_session_approval(&mut self, tool_name: &str, response: crate::approval::ApprovalResponse) {
    ...
    pub fn get_session_approval(&self, tool_name: &str) -> Option<crate::approval::ApprovalResponse> {
    ...
    fn tool_requires_approval(&self, tool_name: &str) -> bool {
        matches!(
            ...
            "run_script" | "shell" | "bash"
        )
    }

    /// Create an ApprovalAction for a tool invocation
    fn create_approval_action(&self, tool_name: &str, input: &serde_json::Value) -> crate::approval::ApprovalAction {
        ...
        match tool_name {
            ...
            "git_commit" | "git_push" | "git_reset" | "git_checkout" => {
                ApprovalAction::GitModify {
                    operation: tool_name.strip_prefix("git_").unwrap_or(tool_name).to_string()
                }
            }
            _ => {
                ApprovalAction::Other {
        ...
        if let Some(ref bridge_arc) = self.remote_bridge {
                    ...
                    .await
            {
                    ...
                    .await
                {
                    ...
                    Err(e) => {
                        tracing::warn!("Remote permission relay failed, falling back to local: {}", e);
    ...
    async fn register_file_ops_executors(&mut self) {
        // read_file
        self.orchestrator.register_executor("read_file", |input| {
            let path = input.get("path")
                .and_then(|v| v.as_str())
        ...
        // list_directory
        self.orchestrator.register_executor("list_directory", |input| {
            let path = input.get("path")
                .and_then(|v| v.as_str())
                .unwrap_or(".");
            let entries: Vec<String> = std::fs::read_dir(path)
                .map_err(|e| format!("Failed to list directory '{}': {}", path, e))?
                .filter_map(|entry| entry.ok().and_then(|e| e.path().to_str().map(|s| s.to_string())))
                .collect();
            Ok(serde_json::to_string_pretty(&entries).unwrap_or_else(|_| format!("{:?}", entries)))
        }).await;

        // write_file
        self.orchestrator.register_executor("write_file", |input| {
            let path = input.get("path")
                .and_then(|v| v.as_str())
        ...
        self.orchestrator.register_executor("edit_file", |input| {
            let path = input.get("path")
                .and_then(|v| v.as_str())
        ...
        // create_directory
        self.orchestrator.register_executor("create_directory", |input| {
            let path = input.get("path")
                .and_then(|v| v.as_str())
        ...
        // delete_file
        self.orchestrator.register_executor("delete_file", |input| {
            let path = input.get("path")
                .and_then(|v| v.as_str())
    ...
    async fn register_search_executors(&mut self) {
        ...
        self.orchestrator.register_executor("search_files", |input| {
        ...
        self.orchestrator.register_executor("search_code", |input| {
            let pattern = input.get("pattern")
                .and_then(|v| v.as_str())
            ...
            let output = std::process::Command::new("rg")
                ...
                .output()
                .or_else(|_| std::process::Command::new("grep").args(["-r", "-n", pattern, path]).output())
    ...
    async fn register_git_executors(&mut self) {
        // git_status
        self.orchestrator.register_executor("git_status", |_input| {
        ...
        // git_diff
        self.orchestrator.register_executor("git_diff", |input| {
        ...
        // git_log
        self.orchestrator.register_executor("git_log", |input| {
        ...
        // git_show
        self.orchestrator.register_executor("git_show", |input| {
    ...
    async fn register_command_executor(&mut self) {
        self.orchestrator.register_executor("execute_command", |input| {
            let command = input.get("command")
                .and_then(|v| v.as_str())
            ...
            let safe_prefixes = ["ls", "cat", "head", "tail", "wc", "grep", "rg", "find", "echo", "pwd", "date", "which", "file", "stat"];
    ...
    async fn register_semantic_search_executors(&mut self) {
        ...
        self.orchestrator.register_executor("query_codebase", |input| {
            let query = input.get("query")
                .and_then(|v| v.as_str())
                .ok_or_else(|| "query_codebase requires 'query' parameter".to_string())?;
            let limit = input.get("limit").and_then(|v| v.as_u64()).unwrap_or(10) as usize;
            let min_score = input.get("min_score").and_then(|v| v.as_f64()).unwrap_or(0.7) as f32;
        ...
        self.orchestrator.register_executor("index_codebase", |input| {
            let path = input.get("path")
                .and_then(|v| v.as_str())
            ...
            rt.block_on(async {
                SemanticSearchTool::execute_index(path).await
            })
        }).await;

        // search_with_filters - advanced search with filters
        self.orchestrator.register_executor("search_with_filters", |input| {
            let rt = tokio::runtime::Handle::try_current()
                .map_err(|_| "No tokio runtime available".to_string())?;

            rt.block_on(async {
                SemanticSearchTool::execute_filtered_search(&input).await
            })
        }).await;

        // get_rag_statistics
        self.orchestrator.register_executor("get_rag_statistics", |_input| {
            let rt = tokio::runtime::Handle::try_current()
                .map_err(|_| "No tokio runtime available".to_string())?;

            rt.block_on(async {
                SemanticSearchTool::execute_get_stats().await
            })
        }).await;

        // search_git_history
        self.orchestrator.register_executor("search_git_history", |input| {
            let rt = tokio::runtime::Handle::try_current()
                .map_err(|_| "No tokio runtime available".to_string())?;

            rt.block_on(async {
                SemanticSearchTool::execute_git_history_search(&input).await
            })
        }).await;
    }
    ...
    async fn register_context_recall_executor(&mut self) {
        self.orchestrator.register_executor("recall_context", |input| {
            let query = input.get("query")
                .and_then(|v| v.as_str())
            ...
            rt.block_on(async {
                ContextRecallTool::execute_recall(query).await
            })
        }).await;
    }
    ...
    async fn register_code_exec_executor(&mut self) {
        self.orchestrator.register_executor("execute_code", |input| {
            ...
            rt.block_on(async {
                CodeExecTool::execute_code_helper(language, code).await
            })
        }).await;
    }
    ...
    pub async fn register_mcp_executors(&mut self) {
        ...
        for tool in mcp_tools {
            ...
            self.orchestrator.register_executor(&tool_name, move |input| {
                let rt = tokio::runtime::Handle::try_current()
                    .map_err(|_| "No tokio runtime available".to_string())?;
                ...
                rt.block_on(async move {
                    ...
                    registry.execute_tool(&name, arguments).await
    ...
    async fn register_agent_pool_executors(&mut self) {
        ...
        self.orchestrator.register_executor("agent_spawn", move |input| {
            ...
            rt.block_on(async move {
                ...
                pool_guard.spawn_agent(task, context, Some(config))
                    .await
                    .map(|agent_id| format!("Spawned background agent '{}' for task: {}", agent_id, desc))
                    .map_err(|e| e.to_string())
            })
        }).await;
        ...
        self.orchestrator.register_executor("agent_status", move |input| {
        ...
        self.orchestrator.register_executor("agent_list", move |_input| {
            let rt = tokio::runtime::Handle::try_current()
                .map_err(|_| "No tokio runtime available".to_string())?;
        ...
        self.orchestrator.register_executor("agent_stop", move |input| {
            let agent_id = input.get("agent_id")
                .and_then(|v| v.as_str())
        ...
        self.orchestrator.register_executor("agent_await", move |input| {
            let agent_id = input.get("agent_id")
                .and_then(|v| v.as_str())
            ...
            rt.block_on(async move {
                let pool_guard = pool.read().await;
                pool_guard.await_completion(&id)
                    .await
                    .map(|result| {
        ...
        self.orchestrator.register_executor("agent_file_locks", move |_input| {
            let rt = tokio::runtime::Handle::try_current()
                .map_err(|_| "No tokio runtime available".to_string())?;
    ...
    async fn register_task_manager_executors(&mut self) {
        ...
        let Some(manager) = &self.task_manager else {
            tracing::debug!("Task manager not configured, skipping task manager executor registration");
            return;
        };
        ...
        self.orchestrator.register_executor("task_create", move |input| {
            let description = input.get("description")
                .and_then(|v| v.as_str())
            ...
            rt.block_on(async move {
                ...
                let task_id = mgr.create_task(desc, parent, TaskPriority::Normal)
                    .await
                    .map_err(|e| e.to_string())?;
        ...
        self.orchestrator.register_executor("task_start", move |input| {
        ...
        self.orchestrator.register_executor("task_complete", move |input| {
            let task_id = input.get("task_id")
                .and_then(|v| v.as_str())
            ...
            rt.block_on(async move {
                let mgr = manager.read().await;
                mgr.complete_task(&id, res).await.map_err(|e| e.to_string())?;
        ...
        self.orchestrator.register_executor("task_fail", move |input| {
            let task_id = input.get("task_id")
                .and_then(|v| v.as_str())
                .ok_or_else(|| "task_fail requires 'task_id' parameter".to_string())?;
            let reason = input.get("reason").and_then(|v| v.as_str()).unwrap_or("Unknown error");
        ...
        self.orchestrator.register_executor("task_tree", move |_input| {
            let rt = tokio::runtime::Handle::try_current()
                .map_err(|_| "No tokio runtime available".to_string())?;
        ...
        self.orchestrator.register_executor("task_ready", move |_input| {
            let rt = tokio::runtime::Handle::try_current()
                .map_err(|_| "No tokio runtime available".to_string())?;
    ...
    async fn register_plan_executor(&mut self) {
        ...
        self.orchestrator.register_executor("plan_task", move |input| {
            ...
            let max_iterations = input.get("max_iterations").and_then(|v| v.as_u64()).unwrap_or(10) as u32;
    ...
    async fn register_fetch_url_executor(&mut self) {
        self.orchestrator.register_executor("fetch_url", |input| {
            ...
            rt.block_on(async move {
                WebTool::fetch_url_content(&url_str).await
                    .map_err(|e| e.to_string())
            })
        }).await;
    ...
    pub async fn execute(&self, tool_use: &ToolUse, context: &ToolContext) -> Result<ToolResult> {
        ...
        let capabilities: Option<brainwires::permissions::AgentCapabilities> = context.capabilities
            .as_ref()
        ...
        if let Some(ref capabilities) = capabilities {
            ...
            if !capabilities.allows_tool(&tool_use.name) {
                ...
                return Ok(ToolResult::error(
                    tool_use.id.clone(),
                    format!("Tool '{}' is not permitted by agent capabilities", tool_use.name)
                ));
            }
            ...
            if capabilities.requires_approval(&tool_use.name) && self.permission_mode == PermissionMode::Auto {
                tracing::debug!(
            ...
            if let Some(path) = tool_use.input.get("path").and_then(|v| v.as_str()) {
                ...
                if is_write && !capabilities.allows_write(path) {
                    tracing::warn!(
                        "Write to path '{}' denied by capability system",
                        path
                    );
                    return Ok(ToolResult::error(
                        tool_use.id.clone(),
                        format!("Write access to '{}' is not permitted by agent capabilities", path)
                    ));
                }

                if !is_write && !capabilities.allows_read(path) {
                    tracing::warn!(
                        "Read from path '{}' denied by capability system",
                        path
                    );
                    return Ok(ToolResult::error(
                        tool_use.id.clone(),
                        format!("Read access to '{}' is not permitted by agent capabilities", path)
                    ));
                }
            }
            ...
            if let Some(url_str) = tool_use.input.get("url").and_then(|v| v.as_str()) {
                ...
                if let Some(domain) = extract_domain_from_url(url_str) {
                    if !capabilities.allows_domain(&domain) {
                        tracing::warn!(
                        ...
                        return Ok(ToolResult::error(
                            tool_use.id.clone(),
                            format!("Network access to '{}' is not permitted by agent capabilities", domain)
                        ));
                    }
                }
            }
        ...
        let agent_id = context.metadata.get("agent_id")
            .cloned()
        ...
        if let Some(ref policy_engine) = self.policy_engine {
            ...
            let file_path = tool_use.input.get("path")
                .and_then(|v| v.as_str())
                .map(|s| s.to_string());
            let domain = tool_use.input.get("url")
                .and_then(|v| v.as_str())
            ...
            let trust_level = context.metadata.get("trust_level")
                .and_then(|s| s.parse::<u8>().ok())
                .unwrap_or(TrustLevel::Medium.as_u8());

            let request = PolicyRequest {
                tool_name: Some(tool_use.name.clone()),
                tool_category: Some(brainwires::permissions::AgentCapabilities::categorize_tool(&tool_use.name)),
                file_path,
                domain,
            ...
            match decision.action {
                PolicyAction::Deny => {
                    ...
                    if let Some(ref audit_logger) = self.audit_logger {
                        let _ = audit_logger.log_denied(
                            ...
                            decision.matched_policy.as_deref().unwrap_or("policy_denied"),
                        );
                    }
                    return Ok(ToolResult::error(
                        tool_use.id.clone(),
                ...
                PolicyAction::Escalate => {
                    ...
                    if self.permission_mode != PermissionMode::Auto {
                        return Ok(ToolResult::error(
                            tool_use.id.clone(),
                            format!("Tool '{}' escalated - requires higher authorization", tool_use.name)
                        ));
                    }
                }
                PolicyAction::Allow => {
        ...
        if tool.requires_approval && self.permission_mode == PermissionMode::ReadOnly {
            return Ok(ToolResult::error(
                tool_use.id.clone(),
                format!("Tool '{}' requires approval but permission mode is read-only", tool_use.name)
            ));
        }
        ...
        let mut needs_approval = (tool.requires_approval || self.tool_requires_approval(&tool_use.name))
        ...
        if needs_approval {
            tracing::info!("Tool '{}' requires approval - requesting from user", tool_use.name);
            ...
            match self.request_approval(
                ...
                Err(e) => {
                    tracing::warn!("Approval request failed for tool '{}': {}", tool_use.name, e);
        ...
        let _lock_bundle = if let Some(acm) = &self.access_control {
            match acm.acquire_for_tool(&agent_id, &tool_use.name, &tool_use.input).await {
        ...
        let result = self.route_tool(&tool_use.id, &tool_use.name, &tool_use.input, context).await;
        tracing::debug!("[ToolExecutor] Tool {} completed: is_error={}", tool_use.name, result.is_error);

        // Track successful file reads for read-before-write enforcement
        if let Some(acm) = &self.access_control {
            if tool_use.name == "read_file" && !result.is_error {
        ...
        if let Some(ref audit_logger) = self.audit_logger {
            let target = tool_use.input.get("path")
                .and_then(|v| v.as_str())
    ...
    pub async fn execute_with_retry(
        ...
        use super::error::{classify_error, ToolOutcome};
    ...
    async fn route_tool(&self, tool_use_id: &str, tool_name: &str, input: &Value, context: &ToolContext) -> ToolResult {
           ...
           tool_name.starts_with("create_directory") {
            ...
            if Self::command_needs_sudo(command) {
                match self.request_sudo_password(command).await {
                    Some(password) => BashTool::execute_with_sudo(tool_use_id, tool_name, input, context, password),
            ...
            self.orchestrator.execute(tool_use_id, tool_name, input, context).await
            ...
            if let Some(session_task_tool) = &self.session_task_tool {
                session_task_tool.execute(tool_use_id, tool_name, input).await
            } else {
                ToolResult::error(
                    tool_use_id.to_string(),
            ...
            if let Some(task_manager_tool) = &self.task_manager_tool {
                task_manager_tool.execute(tool_use_id, tool_name, input).await
            } else {
                ToolResult::error(
                    tool_use_id.to_string(),
            ...
            let working_directory = input.get("working_directory")
                .and_then(|v| v.as_str())
                .unwrap_or(".");
            let build_type = input.get("build_type")
                .and_then(|v| v.as_str())
                .unwrap_or("npm");
...
mod tests {
    ...
    async fn test_route_tool_file_ops() {
        ...
        for tool in file_tools {
            let result = executor.route_tool("test-tool-id", tool, &serde_json::json!({}), &context).await;
    ...
    async fn test_route_tool_bash() {
        ...
        let result = executor.route_tool("test-bash-id", "execute_command", &serde_json::json!({}), &context).await;
        assert!(!result.content.contains("Unknown tool category"));
    }
    ...
    async fn test_route_tool_git() {
        ...
        for tool in git_tools {
            let result = executor.route_tool("test-git-id", tool, &serde_json::json!({}), &context).await;
            assert!(!result.content.contains("Unknown tool category"));
        }
    }
    ...
    async fn test_route_tool_web() {
        ...
        let result = executor.route_tool("test-web-id", "fetch_url", &serde_json::json!({}), &context).await;
        assert!(!result.content.contains("Unknown tool category"));
    }
    ...
    async fn test_route_tool_search() {
        ...
        let result = executor.route_tool("test-search-id", "search_code", &serde_json::json!({}), &context).await;
        assert!(!result.content.contains("Unknown tool category"));
    }
    ...
    async fn test_route_tool_mcp() {
        ...
        let result = executor.route_tool("test-mcp-id", "mcp_test_tool", &serde_json::json!({}), &context).await;
        assert!(!result.content.contains("Unknown tool category"));
    }
    ...
    async fn test_route_tool_unknown_category() {
        ...
        let result = executor.route_tool("test-unknown-id", "completely_unknown", &serde_json::json!({}), &context).await;
        assert!(result.is_error);
        assert!(result.content.contains("Unknown tool category"));
    }
    ...
    fn test_permission_mode_getter() {
        ...
        assert_eq!(executor_readonly.permission_mode(), PermissionMode::ReadOnly);
    }
    ...
    async fn test_approval_flow_no_channel() {
        ...
        let response = executor.request_approval(
            "write_file",
            "Write a file",
