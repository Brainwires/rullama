...
pub async fn handle_plan_action(action: CommandAction) -> Result<bool> {
    match action {
                        ...
                        match LanceDatabase::new(db_str).await {
                            Ok(client) => {
                                ...
                                let plans = if let Some(conv_id) = conversation_id {
                                    plan_store.get_by_conversation(&conv_id).await.unwrap_or_default()
                                } else {
                        ...
                        match LanceDatabase::new(db_str).await {
                            Ok(client) => {
                                ...
                                let plan = if plan_id.len() < 36 {
                                    let all_plans = plan_store.list_recent(100).await.unwrap_or_default();
                                ...
                                if let Some(plan) = plan {
                                    ...
                                    println!("{}\n",
                                        console::style(format!("Plan not found: {}", plan_id)).yellow());
                                }
                            }
                            Err(e) => println!("{}: {}\n", console::style("Error").red().bold(), e),
                        }
                            ...
                            Ok(client) => {
                                ...
                                if let Some(id) = full_plan_id {
                                    match plan_store.delete(&id).await {
                                        Ok(()) => println!("{}\n",
                                            console::style(format!("Plan deleted: {}", &id[..8])).green()),
                                        Err(e) => println!("{}: {}\n",
                                            console::style("Error").red().bold(), e),
                                    }
                                } else {
                            ...
                            Ok(client) => {
                                ...
                                match plan_store.search(&query, 20).await {
                                    Ok(plans) => {
                                        if plans.is_empty() {
                                            println!("{}\n",
                                                console::style(format!("No plans found matching '{}'", query)).yellow());
                                        } else {
                                            println!("{}\n",
                                                console::style(format!("Search results for '{}':", query)).cyan().bold());
                                            for plan in &plans {
                                                println!("  {} - {} [{}]",
                                                    console::style(&plan.plan_id[..8]).green(),
                                                    if plan.title.len() > 40 { format!("{}...", &plan.title[..37]) } else { plan.title.clone() },
                                                    plan.status);
                                            }
                                            println!();
                                        }
        ...
        CommandAction::ActivatePlan(_) => {
            println!("{}\n",
                console::style("Plan activation is only available in TUI mode (--tui).\n\
                Use the TUI for interactive plan execution.").yellow());
            Ok(true)
        }
        ...
        CommandAction::PlanStatus => {
            println!("{}\n",
                console::style("Plan status tracking is only available in TUI mode (--tui).\n\
                Use /plans to list saved plans.").yellow());
            Ok(true)
        }
        ...
        CommandAction::PlanTree(_) => {
            println!("{}\n",
                console::style("Plan branching commands are only available in TUI mode (--tui).").yellow());
            Ok(true)
        }
        _ => Ok(true),
    }
