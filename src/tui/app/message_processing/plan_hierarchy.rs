...
impl App {
                                ...
                                Ok(plans) => {
                                    if plans.is_empty() {
                                        ...
                                        for plan in &plans {
                                            ...
                                            lines.push(format!("    Created: {} | Task: {}...",
                                                created,
                                                &plan.task_description[..50.min(plan.task_description.len())]
                                            ));
                                        }
                                        lines.push("".to_string());
                                        lines.push("Use /plan:show <id> to view details".to_string());
                                        lines.join("\n")
                                    }
                                }
    ...
    pub(super) async fn handle_branch_plan(&mut self, branch_name: String, task_description: String) -> Result<()> {
        use crate::config::PlatformPaths;
        use crate::storage::{EmbeddingProvider, LanceDatabase, PlanStore, VectorDatabase};
        ...
        let parent_info = self.active_plan.as_ref().map(|p| {
            let branch = p.create_branch(
                ...
                format!("Sub-plan for: {}\n\n[Plan content to be generated]", task_description),
            );
        ...
        let (content, new_active_plan) = if let Some((branch, updated_parent, parent_id_short)) = parent_info {
            // Save the branch
            let result = match PlatformPaths::conversations_db_path() {
                Ok(db_path) => {
                    if let Some(db_str) = db_path.to_str() {
                        match LanceDatabase::new(db_str).await {
                            Ok(client) => {
                                ...
                                let embeddings = match EmbeddingProvider::new() {
                                    Ok(e) => Arc::new(e),
                                    Err(e) => { return Err(anyhow::anyhow!("Failed to create embedding provider: {}", e)); }
                                };
                                let _ = client.initialize(embeddings.dimension()).await;
                                let plan_store = PlanStore::new(client, embeddings);
            ...
            ("No active plan. Use /plan:activate <id> to activate a plan first.".to_string(), None)
        };
                        ...
                        Ok(client) => {
                            ...
                            if let Some(mut plan) = plan_to_merge {
                                if plan.parent_plan_id.is_none() {
                                    "Cannot merge a root plan. Only branch plans can be merged.".to_string()
                                } else if plan.merged {
                                ...
                                "No plan to merge. Specify a plan ID or activate a branch plan.".to_string()
                            }
                        }
                        Err(e) => format!("Failed to access database: {}", e),
                    }
                        ...
                        Ok(client) => {
                            ...
                            let root_id = if let Some(id) = plan_id {
                                ...
                                return {
                                    self.messages.push(TuiMessage {
                                        role: "system".to_string(),
                                        content: "No plan specified and no active plan.".to_string(),
                                        created_at: chrono::Utc::now().timestamp(),
                                    });
                                    self.clear_input();
                                };
                            ...
                            match plan_store.get_hierarchy(&root_id).await {
                                Ok(plans) => {
                                    if plans.is_empty() {
                                        ...
                                        let mut lines = vec!["Plan Hierarchy:".to_string(), "".to_string()];

                                        for plan in &plans {
                                            let indent = "  ".repeat(plan.depth as usize);
                                            let branch_label = plan.branch_name.as_deref().unwrap_or("");
                                        ...
                                        lines.push("".to_string());
                                        lines.push("Legend: ▶ active  ✓ merged  ○ other".to_string());
                                        lines.join("\n")
                                    }
                                }
